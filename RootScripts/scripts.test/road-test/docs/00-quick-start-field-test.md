...
sequenceDiagram
    ...
    Dev->>Dev: 2. Build APK (./RootScripts/scripts.test/road-test/scripts/build-apk.ps1 หรือใช้ road-test.ps1)
...
- [ ] **Road Test Master Control:** สามารถสั่งการผ่านสคริปต์หลักได้ทันที
  ```powershell
...
- [ ] **Docker Test Server:** ทุก Service กำลังทำงานและมีสถานะ Healthy
  ...
  ```
- [ ] **Cloudflare Tunnel:** หน้าต่าง Terminal ที่รันคำสั่ง Cloudflare Tunnel **ต้องเปิดค้างไว้ตลอดการทดสอบ**
  ```bash
  powershell ./RootScripts/scripts.test/road-test/road-test.ps1 tunnel
  # หรือ: docker run --rm -it --network=host cloudflare/cloudflared:latest tunnel --url http://localhost:80
  ```
